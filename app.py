...
# --- Binance Data Function (FUTURES API with PROXY BYPASS) ---
def get_binance_futures_data(symbol, interval='15m', limit=100):
    if not symbol.endswith('USDT'):
        symbol = symbol + 'USDT'
    
    # Asli Binance Futures ka link
    binance_url = f"https://fapi.binance.com/fapi/v1/klines?symbol={symbol}&interval={interval}&limit={limit}"
    
    # 2 Free Proxies (America ka block torne ke liye)
    proxies = [
        f"https://api.allorigins.win/raw?url={binance_url}",
        f"https://corsproxy.io/?{binance_url}"
    ]
...
st.markdown("<p style='text-align: center; font-size: 18px; color: #a6a6a6; font-style: italic;'>Premium Binance Futures Chart Bot</p>", unsafe_allow_html=True)
...
if st.button("Detect Coin, Arrow & Generate Chart"):
    if not post_text:
        ...
        if not coin_symbol:
            ...
            with st.spinner(f"Fetching Futures data for {coin_symbol}..."):
            ...
            if df is not None:
                fig, axlist = mpf.plot(
                    ...
                    type='candle', 
                    style='charles', 
                    title=f"{real_symbol} Futures Setup", 
                    returnfig=True,
                    figsize=(8, 5)
                )
                ...
                st.error(f"Error: {coin_symbol} ka data nahi mila. Proxies block ho gaye hain ya API down hai.")
